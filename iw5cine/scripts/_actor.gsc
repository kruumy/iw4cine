...
ActorSpawn()
{
	...
	dvar = "mvm_actor_spawn";
	prompt = "Spawn an actor - ^9[body head]";
	setDvar(dvar, prompt);
	for(;;)
	{
		wait 0.1;
		if(getDvar(dvar) == prompt) continue;
...
ActorModel()
{
	...
	dvar = "mvm_actor_model";
	prompt = "Change model - ^9[actor body head]";
	setDvar(dvar, prompt);
	for(;;)
	{
		wait 0.1;
...
ActorHP()
{
	...
	dvar = "mvm_actor_health";
	prompt = "Health amount - ^9[actor health]";
	setDvar(dvar, prompt);
	for(;;)
	{
		wait 0.1;
...
ActorDelete()
{
	...
	dvar = "mvm_actor_delete";
	prompt = "Delete an actor - ^9[actor]";
	setDvar(dvar, prompt);
	for(;;)
	{
		wait 0.1;
...
ActorNormAnim()
{
	...
	dvar = "mvm_actor_anim";
	prompt = "Actor base animation - ^9[actor anim]";
	setDvar(dvar, prompt);
	for(;;)
	{
		wait 0.1;
...
ActorPlayFX()
{
	...
	dvar = "mvm_actor_fx";
	prompt = "Play FX on tag - ^9[actor tag effect]";
	setDvar(dvar, prompt);
	for(;;)
	{
		wait 0.1;
...
ActorDeathAnim()
{
	...
	dvar = "mvm_actor_death";
	prompt = "Actor death animation - ^9[actor anim]";
	setDvar(dvar, prompt);
	for(;;)
	{
		wait 0.1;
...
ActorEquip()
{
	...
	dvar = "mvm_actor_weapon";
	prompt = "Actor weapon - ^9[actor tag weapon camo]";
	setDvar(dvar, prompt);
	for(;;)
	{
		wait 0.1;
		...
		if(isDefined(actors))
		{
			foreach (actor in actors)
			{
				...
				weaponModel = getweaponmodel(arguments[2]);
				if( isDefined(arguments[3]) && arguments[3] != "" /*&& CanWeaponHaveCamo(arguments[2]) && IsValidCamo(arguments[3])*/ )
				{
					weaponModel = weaponModel + "_" + arguments[3];
				}
				...
				{
					actor.equ[arguments[1]] setModel( weaponModel );
					for (i = 0; i < actorWeaponHideTagList.size; i++) 
					{
						actor.equ[arguments[1]] HidePart(actorWeaponHideTagList[i], weaponModel );
					}
					self iPrintLn("[" + actor.name + "] : ^8" + weaponModel + " ^7attached to ^8" + arguments[1] );
				}
...
ActorNormWalkActorBack()
{
	...
	dvar = "mvm_actor_walk_actorback";
	prompt = "Toggle walk on actorback - ^9[actor]";
	setDvar(dvar, prompt);
	for(;;)
	{
		wait 0.1;
...
ActorFollow()
{
	...
	dvar = "mvm_actor_follow";
	prompt = "Follow the player - ^9[actor time]";
	setDvar(dvar, prompt);
	for(;;)
	{
		wait 0.1;
...
ActorNormWalk()
{
	...
	dvar = "mvm_actor_walk";
	prompt = "Walk to dir - ^9[actor time dir]";
	setDvar(dvar, prompt);
	for(;;)
	{
		wait 0.1;
...
ActorNormWalkAutoStop()
{
	...
	dvar = "mvm_actor_walk_autostop";
	prompt = "Toggle stop actor on death - ^9[actor]";
	setDvar(dvar, prompt);
	for(;;)
	{
		wait 0.1;
...
ActorTeleport()
{
	...
	dvar = "mvm_actor_move";
	prompt = "Teleport actor to you - ^9[actor]";
	setDvar(dvar, prompt);
	for(;;)
	{
		wait 0.1;
...
ActorBack()
{
	...
	dvar = "actorback";
	prompt = "[ Resets all actors' postiions ]";
	setDvar(dvar, prompt);
	for(;;)
	{
		wait 0.1;
...
ActorGoPro()
{
	...
	dvar = "mvm_actor_gopro";
	prompt = "GoPro - ^9[action/actor tag x y z r p y]";
	setDvar(dvar, prompt);
	for(;;)
	{
		wait 0.1;
...
ActorSetPath()
{
	...
	dvar = "mvm_actor_path_save";
	prompt = "Save node for bezier - ^9[actor node]";
	setDvar(dvar, prompt);
	for(;;)
	{
		wait 0.1;
...
ActorDeletePath()
{
	...
	dvar = "mvm_actor_path_del";
	prompt = "Delete node for bezier - ^9[actor node]";
	setDvar(dvar, prompt);
	for(;;)
	{
		wait 0.1;
...
ActorDoPath()
{
	...
	dvar = "mvm_actor_path_walk";
	prompt = "Actor follows bezier - ^9[actor time]";
	setDvar(dvar, prompt);
	for(;;)
	{
		wait 0.1;
...
ActorRename()
{
	...
	dvar = "mvm_actor_rename";
	prompt = "Change actor name - ^9[actor newname]";
	setDvar(dvar, prompt);
	for(;;)
	{
		wait 0.1;
